...
pub fn get_entry_info(scheduler: &mut Scheduler, state: &mut RegisterState) -> SystemCallStatus {
    ...
    let Ok(info_type) = tungsten_kit::syscall::OSDTEntryInfoType::try_from(state.rdx) else {
        return SystemCallStatus::MalformedData;
    };
    let data = match info_type {
        tungsten_kit::syscall::OSDTEntryInfoType::Parent => postcard::to_allocvec(&dt_entry.parent),
        tungsten_kit::syscall::OSDTEntryInfoType::PropertyNamed => {
